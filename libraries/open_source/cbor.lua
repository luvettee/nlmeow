...
local b_rshift = (ok and bit.rshift) or function (a, b) return m_max(0, m_floor(a / (2 ^ b))); end;
...
encoder["nil"] = function() return " nil"; end
...
local function decode(s, opts)
	...
	function fh:read(bytes)
		...
		if #ret < bytes then
			s = s:sub(pos);
			pos = 1;
			local new_data = more(bytes - #ret);
			if not new_data or type(new_data) ~= "string" or #new_data == 0 then
				error "input too short";
			end
			s = s .. new_data;
	...
	-- this function is unused in this implementation, but kept for api compatibility
