...
local avatar = {}
function avatar.get(steamid,avatarsize)
    local huy = nil
    local counter = 4
    local rgba_image = {}
    local huy = nil
    local handle = 0
    if size == 1 then
        handle = native_ISteamFriends_GetSmallFriendAvatar( native_ISteamFriends , tonumber(steamid:sub(4, -1)) + 76500000000000000ULL)
    elseif size == 2 then
        handle = native_ISteamFriends_GetMediumFriendAvatar( native_ISteamFriends , tonumber(steamid:sub(4, -1)) + 76500000000000000ULL)
    elseif size == 3 then
        handle = native_ISteamFriends_GetLargeFriendAvatar( native_ISteamFriends , tonumber(steamid:sub(4, -1)) + 76500000000000000ULL)
    end
    ...
    local image_loaded = render.load_image(image_bytes ,  vector(128,128))

    return image_loaded
end
function avatar.get_steamid_for_playerindex(index)
    return panorama.GameStateAPI.GetPlayerXuidStringFromEntIndex(ent_idx)
end
