local render_texture, render_load_image_rgba, string_format, table_insert, bit_band, bit_rshift, bit_lshift, string_char, ffi_new, ffi_copy, ffi_fill, ffi_string = render.texture, render.load_image_rgba, string.format, table.insert, bit.band, bit.rshift, bit.lshift, string.char, ffi.new, ffi.copy, ffi.fill, ffi.string
...
local function gifdecode(self)
	...
	while true do
		...
		elseif blocktype == GIF_IMG_BLOCK then
			...
			local texture = render_load_image_rgba(data_str, vector(iw, ih))
...
local function gifdrawframe(self, frame, position, size, g_color, flags, rounding)
	if self.frames[frame] == nil then
		error("Frame not found", 2)
	end

	local texture = gifframe(self, frame)
	render_texture(texture, position, size or vector(self.width, self.height), g_color or color(), flags or "f", rounding or 0)
end
