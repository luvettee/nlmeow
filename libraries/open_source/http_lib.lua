-- #region ffi
local steam_http_raw = ffi.cast("uint32_t**", ffi.cast("char**", ffi.cast("char*", utils.opcode_scan("client.dll", "B9 ? ? ? ? E8 ? ? ? ? 83 3D ? ? ? ? ? 0F 84")) + 1)[0] + 48)[0] or error("steam_http error")
...
events.render:set(function ()
	for _, instance in ipairs(requests) do
		if globals.curtime - instance.ls > instance.task_interval then
			instance:_process_tasks()
			instance.ls = globals.curtime
		end
	end
end)
...
function http.new(task)
	task = task or {}
	local instance = setmetatable({requests = {}, task_interval = task.task_interval or 0.3, enable_debug = task.debug or false, timeout = task.timeout or 10, ls = globals.curtime}, http_mt)
