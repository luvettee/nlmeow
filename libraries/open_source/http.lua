---@diagnostic disable: need-check-nil, undefined-field, inject-field
--
-- dependencies
--
...
local string_format, string_lower, string_len = string.format, string.lower, string.len
...
do
    ...
    local function callback_base_run_common(self, param, io_failure)
        ...
        xpcall(function()
            ...
            if handler ~= nil then
                xpcall(handler, print_error, param, io_failure)
            end
    ...
    SteamAPI_RegisterCallResult = find_sig("steam_api.dll", "55 8B EC 83 3D CC CC CC CC CC 7E 0D 68 CC CC CC CC FF 15 CC CC CC CC 5D C3 FF 75 10", "void(__cdecl*)(struct SteamAPI_callback_base *, uint64_t)")
    ...
    SteamAPI_RegisterCallback = find_sig("steam_api.dll", "55 8B EC 83 3D CC CC CC CC CC 7E 0D 68 CC CC CC CC FF 15 CC CC CC CC 5D C3 C7 05", "void(__cdecl*)(struct SteamAPI_callback_base *, int)")
    SteamAPI_UnregisterCallback = find_sig("steam_api.dll", "55 8B EC 83 EC 0C 80 3D CC CC CC CC CC 0F", "void(__cdecl*)(struct SteamAPI_callback_base *)")

    steam_client_context = find_sig(
        "client.dll",
        " B9 CC CC CC CC E8 CC CC CC CC 83 3D CC CC CC CC CC 0F 84",
        "uintptr_t",
        1, 1
    )
...
local function http_request_callback_common(req, callback, successful, data, ...)
    ...
    -- run callback
    is_in_callback = true
    xpcall(callback, print_error, successful, data, ...)
...
M.get(
    "https://fontawesome.com/images/favicon/icon.svg",
    function(success, response)
        print_raw("success:", success)
        print_raw("response:")
        for key, value in pairs(response) do
            print_raw("key:", key, "\x20value:", value)
        end
    end
)
return M
